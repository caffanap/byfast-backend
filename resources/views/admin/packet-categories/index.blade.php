        ...
        <div class="px-4 py-3 mb-0 border-0 rounded-t">
            <div class="flex flex-wrap items-center">
                <div class="relative flex justify-between flex-1 flex-grow w-full max-w-full px-4">
                    ...
                    <a href="{{ route('admin.packet-categories.create') }}" class="px-4 py-2 mr-1 text-xs font-bold text-white uppercase transition-all duration-150 ease-linear bg-pink-500 rounded shadow outline-none active:bg-pink-600 hover:shadow-md focus:outline-none">
                        Add
                    </a>
                </div>
            </div>
            ...
            <table class="items-center w-full bg-transparent border-collapse" id="schools-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left">
                            {{$packetCategory->description}}
                        </td>
                        <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left">
                            {{$packetCategory->created_at}}
                        </td>
                        <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left">
                            {{$packetCategory->updated_at}}
                        </td>
                        <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left">
                            <div class="flex p-2 text-xs align-middle border-t-0 border-l-0 border-r-0 whitespace-nowrap">
                                <a href="{{ route('admin.packet-categories.show',$packetCategory->id) }}" class="p-1 px-2 mx-1 text-xs text-white bg-green-500 rounded-md">Show</a>
                                <a href="{{ route('admin.packet-categories.edit',$packetCategory->id) }}" class="p-1 px-2 mx-1 text-xs text-white bg-yellow-500 rounded-md">Edit</a>
                                <form action="{{ route('admin.packet-categories.destroy', $packetCategory->id) }}" method="POST">
                                    @csrf
                                    @method("DELETE")
